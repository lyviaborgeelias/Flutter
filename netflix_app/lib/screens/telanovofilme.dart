...
class _TelaNovoFilmeState extends State<TelaNovoFilme> {
  ...
  void salvarDados() async {
    ...
    String filme =
        "${nomeFilme.text}|${anoFilme.text}|${descricaoFilme.text}|${imagemFilme.text}";
    //1º Clona a lista do banco
    List<String> lista = banco.getStringList("filmes") ?? [];
    //2º Adiciona o novo filme
    lista.add(filme);
    //3º Substitui a lista do banco
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Criar Filme", style: TextStyle(color: Colors.white)),
      ...
      body: Center(
        child: Column(
          children: [
            ...
            MeuTextField(
              controlador: descricaoFilme,
              tipo: "descricao do filme",
            ),
